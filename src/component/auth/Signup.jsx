...
const Signup = () => {
  ...
  const handleSignup = async () => {
    ...
    try {
      setLoading(true);
      const [responseData, fetchError] = await useAxios('POST', 'auth/register', null, formData);
      if (responseData) {
        ...
        toast.error(fetchError?.message || 'Signup failed', { autoClose: 2000 });
      }
    } catch (err) {
      ...
      <div className="lg:w-[50%] w-full flex flex-col items-center px-3 md:px-6 py-6 lg:py-12 lg:px-8">
        <div className="w-full max-w-md space-y-8 bg-white p-5 rounded-xl shadow-lg">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button className="flex items-center justify-center py-2.5 px-4 border rounded-lg bg-white hover:bg-gray-50" onClick={handleGoogleLogin}>
              <FaGoogle className="text-red-500 mr-2" />
              Google
            </button>
            <button className="flex items-center justify-center py-2.5 px-4 border rounded-lg bg-white hover:bg-gray-50" onClick={handleFacebookLogin}>
          ...
          <p className="text-sm text-gray-600 text-center mt-6">
            ...
            <Link to="/" className="text-primary-600 hover:text-brand-500 ml-1">Login</Link>
          </p>
        </div>
